...
delete FROM {QueueTable.Identifier} WITH (READPAST, ROWLOCK) WHERE id IN (select id from {Parent.MessageStorageSchemaName}.{DatabaseConstants.IncomingTable} WITH (NOLOCK));
